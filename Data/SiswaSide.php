 ...
 */
 $aColumns = array( 'no','induk', 'nama', 'kelas', 'hafalan'); //Kolom Pada Tabel
 
 // Indexed column (used for fast and accurate table cardinality)
 $sIndexColumn = 'no';
 
 // DB table to use
 $sTable = 'siswa_hafalan'; // Nama Tabel
...
//  $btn = '<a href="#" onClick="showModals('.$aRow['no'].')">Edit</a> | <a href="#" onClick="deleteUser('.$aRow['no'].')">delete</a>';
  ...
  if(count($select_data) >0){
    $select = "<select class='form-control'>";
        foreach($select_data as $select_dat){
            $select = $select."<option value = '$select_dat'>$select_dat</option>\n";
        }
  ...
  $row = array( $btn, $aRow['no'], $aRow['induk'], addslashes($aRow['nama']), $aRow['kelas'], $select);
  $output['aaData'][] = $row;
 }

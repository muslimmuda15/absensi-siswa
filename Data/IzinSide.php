 ...
 */
 $aColumns = array( 'no','induk', 'nama', 'kelas', 'tanggal', 'keterangan', 'guru' ); //Kolom Pada Tabel
 
 // Indexed column (used for fast and accurate table cardinality)
 $sIndexColumn = 'no';
 
 // DB table to use
 $sTable = 'izin'; // Nama Tabel
...
//       . "<button type='delete' onClick=\"print()\" class='glyphicon glyphicon-print btn btn-primary'></button>";
  for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
   $row[] = $aRow[ $aColumns[$i] ];
  }
  $row = array( $btn/*."<label style='display:none'>".$aRow['no']."</label>"*/, $aRow['no'], $aRow['induk'], addslashes($aRow['nama']), $aRow['kelas'], $aRow['tanggal'], $aRow['keterangan'], $aRow['guru'] );
  $output['aaData'][] = $row;
 }

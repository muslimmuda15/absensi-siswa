...
if(getCookies() == TRUE)
{
 ...
 switch ($_POST['type']) {
  ...
  case "get":
   ...
   $SQL = mysqli_query($con, "SELECT * FROM siswa_hafalan WHERE no='".$_POST['no']."'");
  ...
  case "new":
      ...
      $cek=array('status'=>'','errorMessage'=>'');
        ...
        {
            {
                $sql_siswa = "SELECT * from siswa_hafalan where induk like '".$_POST["induk"]."'";
...
//                        $getinduk = $row["induk"];
//                    }
                    $result = mysqli_query($con, "SELECT MAX(no) FROM siswa_hafalan");
                    ...
                    $cek=array('status'=>'invalid','errorMessage'=>$_POST["induk"].' sudah ada dalam database');
            }
        }
...
//        $cek=array('status'=>'invalid','errorMessage'=>'Sepertinya nama siswa dan kelasnya tidak sesuai');
  ...
  case "delete":
    {
       $SQL = mysqli_query($con, "DELETE FROM siswa_hafalan WHERE no='".$_POST['no']."'");
       if($SQL){
